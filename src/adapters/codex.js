...
async function makeFileOperation(root, id, target, content, sourceModule, reason, managedDigest, risk = 'low') {
  ...
  return operation({ id, action: prior === ABSENT ? 'create-file' : 'replace-file', target,
    expectedPriorDigest: prior, resultingDigest: sha256(content), content, managedDigest, sourceModule, adapter: 'codex', reason, risk });
}
...
export async function plan(profile, context) {
  ...
  operations.push(await makeFileOperation(root, 'codex:global-instructions', 'AGENTS.md', global,
    'profile', 'Projects the enabled operating instructions for Codex without replacing unmanaged instructions.', sha256(block)));
  for (const capability of enabled(profile, 'capability')) {
    ...
    operations.push(await makeFileOperation(root, `codex:capability:${capability.id}`, target, content, capability.id,
      `Projects neutral capability ${capability.id} as Codex's SKILL.md entrypoint.`));
